...
#library(readr)
Airbnb_Sydney <- read.csv("Airbnb Sydney.csv")
...
# I will remove any weird characters like dollar signs from (potentially) numeric columns. I will look at all data types. The date columns may need to be formated, or broke up for more detailed analysis (for instance to look at a single year, month, or day). All classes of the columns should be doubled checked, because they may need to be re-assgined to be an integer from a character, for instance. I would double check that the data was read in correctly with the right headers. To analyze text data, I would parse and remove any strange characters to understand what words or how many words are in a cell. 

# Q2

#I looked at the overall distribution of the dataframe and decided to focus, at first, on the the property type, the city, price and host since columns. I felt these variables would offer the most insight into the dataset. 
...
# Looking closer at the these columns, I see that have a large number of cities and property types to work with as character and factor variables. I also found an "Other" type in the city variable, which I have decided to leave in for now, but may turn into an NA. However, I don't think "other" means the same thing as NA, so I will decide on a case by case basis on what to do with that. I also looked at price. I removed the number sign from the data so I could look at it more closely. I also looked at the distribution and can see that the majority of the prices are at or below 200 dollars. I also looked at host_since, and we can see that we have data from 2018 to 2009 to work with. I also looked at cleaning fees, super host status, and review scores.
...
#Taking a look at this relationship visually, we can see that the two variables are closely related, with a higher number overall reviews being correlated with a higher percentage of reviews per month. This does suggest some colinearity between the two variables. I also looked at the distriubtion of both variables, and they look similar. 
...
#2. 2. I think that hosts who have been using Aribnb longer will have more reviews and higher review ratings. I will use the host_since date variable, number of reviews, and ratings to investigate this. I think super host status will need to be held constant, as well as price, location, property type, and beds.
...
#Below, I have sorted the number of reviews by property type, and sorted them by there average review rating. We can see that apartments have the highest average reivews, and most total number of reivews. After that, we can see that islands and chalet's also have a high average reviews, but not very many total reviws. Apartments have most reviews overall, but not the highest average. Most of the property types in the data are apartments, so this makes sense. 
...
#The first and visual shows us that number of reviews by cleaning fee, and see can see the data is very skewed, with more reviews per month with less cleaning fees. There is a similar trend with the total number of reviews and cleaning fee. There is a steep drop in the number of reviews after 100 to 150 dollars. To look at this from another angle, I plotted property type by cleaning fee, and there are three property types that stand out in terms of cleaning fee- house, villa and apartment.
...
clean_model
plot(clean_num~Airbnb_Sydney$reviews_per_month)
...
plot(host_since_date~host_response_num, col = colors[Airbnb_Sydney$host_is_superhost],
...
#I chose to look at cities, because I thought zipcodes would include a wider area, which could introduce more variability in the data. Cities seemed to away to hold variables like price constant. 
...
city_agg = aggregate(city_df$weights, 
                     by=list(city = city_df$city, number_of_reviews = city_df$number_of_reviews, 
                             review_rating = city_df$review_scores_rating), 
                     mean)
...
#My analysis focused on the verfication types, price, property types, super host, cleaning fee, and review ratings, and number of reviews. I found a number of interesting patterns:
...
#3. Location and property types significantly affects how often a listing is rented. Certain cities and property types where listed way more than others. Even location or property specific words in the listing description contributed to a difference in review numbers and ratings. I would recommend that hosts use certain words to attract more customers in their description. 
...
#I started the data cleaning process with some obvious fixes such as removing the dollar sign from price and converting the host_since variable to a proper date format. I also cleaned several of the character and list variables to do text analysis. This phase also required going back and reviewing my data explortion phase to decide what columns to focus on and what class they needed to be. Cleaning the data allowed me to aggregate and group the data to start looking for potential relationships between the data. 
#The data analysis phase followed next to further investigate potential relationships, with more data visualization to illustrate what was found. This was part of the data visualization and presentation phase of the lifecycle. After compiling the analysis, I created visuals to show the nature of the relationships I found and show trends in the data. This phase included raw numbers from the analysis and data visualizations. The data modeling phase was touched on briefly in this project as we did some basic statistics. This analysis leaned toward the generative analysis side of things. 
